...
def split_genome_into_genes(genome: str) -> list[str]:
  ...
  # output is 4 bits
  return [[genome[i],genome[i+1:i+bits_per_gate+1],genome[i+bits_per_gate+1:i+bits_per_gene]] for i in range(0, len(genome)-4, bits_per_gene)] + [[genome[-4:]]]
...
for g in range(generations):
  ...
  if (g + 1) % log_step == 0:
    print(f'[Generation {g + 1}] Fitness: {avg_fitness}')
